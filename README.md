...
A local Flask client for `gemma`, retaining ECM-paced text, SQLite history, and the optional semantic cache while adding a small podcast studio.
...
Gemma Chat supports an OpenAI-compatible local server (including llama.cpp-style
servers) on loopback port 8080 or 8081. The app adds `/v1` automatically:

```bash
export GEMMA_LOCAL_LLM_URL='http://127.0.0.1:8080'
export GEMMA_LOCAL_LLM_MODEL='your-local-model-name'
python app.py
```
...
require `OPENAI_API_KEY`. You can also set `GEMMA_LOCAL_LLM_API_KEY` if your local
...
terminal open while using Gemma Chat.
...
- **Speak Aloud** generates and locally caches the selected Gemma voice.
- The **Pacing** slider controls text speed and pitch-preserved audio playback speed.
- **Save Recording** exports an individual Gemma response at the current paced speed.
- **Export Podcast** gives user prompts the Host voice and Gemma responses the Gemma voice. It now derives both voice tempo and inter-turn pause timing from the Pacing slider, normalizes the finished program to approximately -16 LUFS, and exports a VBR MP3 whose filename includes the applied speed.
...
- **Response Length** guides Gemma from very concise through expansive answers.
